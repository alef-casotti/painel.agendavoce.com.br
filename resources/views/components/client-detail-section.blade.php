...
<div {{ $attributes->merge(['class' => 'relative overflow-hidden bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col']) }}>
    <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3 shrink-0">
        <div class="flex w-10 h-10 items-center justify-center rounded-xl {{ $styles['wrap'] }} {{ $styles['icon'] }}">
            @switch($icon)
                @case('mail')
